...
Coordinate systems are mathematical tools used to represent points in space or on a surface using a set of numerical values. They provide a systematic way to describe the location of points relative to a reference point or axis. There are several types of coordinate systems, each with its unique set of parameters and applications.
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=1.5]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{\footnotesize 2-D and 3-D Cartesian Coordinate Systems with a single point located at one unit in each axes. \label{fig:Cartesian Coordinates}}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{\footnotesize LEFT: The point $(x, y)$ in Cartesian coordinates can be represented as $(r, \theta)$ in polar coordinates, where $r$ is the distance from the origin to the point and $\theta$ is the angle measured counterclockwise from the positive $x$-axis. RIGHT: The point $(x, y, z)$ in Cartesian coordinates can be represented as $(r, \theta, \phi)$ in a modified spherical coordinate system, where $r$ is the distance from the origin to the point, $\theta$ is the angle measured counterclockwise from the positive $x$-axis towards the $y$ direction and $\phi$ is the angle from the $z$-axis.}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=1]
		...
		\draw[dashed] (0,0,1.75) -- (3,1.8,1.75) node[midway, above left] {$r$};
		\draw[dotted] (3.5,0,0) -- (3.5,0,1.75) node[midway, right] {$z$};
		\draw[dotted] (3.5,0,1.75) -- (3.5,0,3.5);
		\draw[dotted] (3.5,0,1.75) -- (0,0,1.75) -- (0,3.5,1.75);
		\draw[dotted] (0,3.5,0) -- (0,3.5,1.75) node[midway, above] {$z$};	
		\draw[dotted] (0,3.5,1.75) -- (0,3.5,3.5);
		\draw[dotted] (3.5,0,0) arc (0:90:3.5);
		\draw[dotted] (3.5,0,1.75) arc (0:90:3.5);
		\draw[-] (1.5,0,1.75) arc (0:30:1.5) node[midway, right] {$\theta$};
		\draw[dotted] (3.5,0,3.5) arc (0:90:3.5);
	\end{tikzpicture}	
\caption{\footnotesize The point $(x, y, z)$ in Cartesian coordinates can be represented as $(r, \theta, z)$ in cylindrical coordinates, where $r$ is the distance from the $z$ axis to the point and $\theta$ is the angle measured counterclockwise from the positive $x$-axis. }
\label{fig:cylindrical coords}
\end{figure}

\keyword{Cylindrical coordinates} (see figure \ref{fig:cylindrical coords}) are a system used to represent points in three-dimensional space using three parameters: radial distance $r$, polar angle $\theta$, and height $z$. In this system, the radial distance measures the distance from the $z$-axis to the point in a plane perpendicular to the $z$-axis. The polar angle $\theta$ represents the azimuthal angle, measured counterclockwise from the positive $x$-axis to the projection of the point on the $xy$-plane. The height $z$ indicates the distance of the point along the z-axis. Cylindrical coordinates are particularly useful for describing objects or phenomena with cylindrical symmetry. The conversion between cylindrical coordinates and Cartesian coordinates can be done using trigonometric functions.
