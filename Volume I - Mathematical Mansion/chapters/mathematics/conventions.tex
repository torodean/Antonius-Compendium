...
Given a complex number $z=a+bi$ (where $a,b \in \mathbb{R}$), the exponential $e^z$ is most common explained with \keyword{Euler's Formula}. First, note that $e^z = e^{a+bi} = e^ae^{bi}$ by the simple properties of exponents. Since $e^a$ is real, we can say this is simply some constant $A$, giving $Ae^{bi}$. The important term here is $e^{ib}$. If expanded as a \keyword{Taylor series}, we have
\begin{align}
	e^{ib} = 1 +ib + \frac{(ib)^2}{2!} + \frac{(ib)^3}{3!} + \frac{(ib)^4}{4!} + \cdots
\end{align}

The Taylor series for $\sin(b)$ and $\cos(b)$ can be found in this series via a bit of rearranging. Since $i^2=-1$, $i^3 = -i$, $i^4=i$, and so on, this can be written as
...
\begin{defn}[Euler's Formula \label{Euler's Formula Definition}]{1}
\keyword{Euler's formula} was named after Leonhard Euler. It is a mathematical formula which establishes the fundamental relationship between the trigonometric functions and the complex exponential function.
\begin{align}
